...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        if (isset($_SESSION['daySettingChange']) && $_GET['edit'] == '1') {
            ...
            $queryCreatePull = "SELECT * FROM `day-settings`";
            $resultCreatePull = mysqli_query($db, $queryCreatePull); //or die('Error: ' . mysqli_error($db) . ' with query ' . $queryCreatePull);
...
if (isset($_SESSION['daySettingChange'])) {
    initializeHead('..', 'Daginstelling wijzigen bij Rasa Senang', false, false, true, false);
} else {
    initializeHead('..', 'Nieuwe Daginstelling voor Rasa Senang', false, false, true, false);
}
